...
#  Keys     : Loaded from config.json (prompt via menu, never hardcoded)
#  Primary  : OpenRouter → google/gemini-3.1-pro-preview  (via requests)
#  Fallback : Google AI Studio → gemini-1.5-flash         (via requests)
#  Deps     : ONLY requests — no Rust, no compilation, Termux-safe ✅
# ─────────────────────────────────────────────────────────────────────────────

import requests   # stdlib-level: pip install requests  (pure Python, no Rust)
...
# OpenRouter endpoint (OpenAI-compatible REST)
...
# ── Optional heavy deps (pixel-reflex only) ───────────────────────────────────
try:
...
# ─────────────────────────────────────────────────────────────────────────────
#  AI CALLERS — pure HTTP via requests (no Rust deps)
# ─────────────────────────────────────────────────────────────────────────────

def _call_openrouter(api_key: str, prompt: str, frame_b64: str | None = None) -> str | None:
    """Call OpenRouter (Gemini 3.1 Pro) via REST. Vision-capable."""
    content = []
    if frame_b64:
...
def _call_google(api_key: str, prompt: str, frame_b64: str | None = None) -> str | None:
    """Call Google Gemini 1.5 Flash via REST. Vision-capable."""
    parts = []
    if frame_b64:
...
def call_ai(cfg: dict, prompt: str, frame_bytes: bytes | None = None) -> str | None:
    """Try OpenRouter first, Google fallback, return text response or None."""
    b64 = base64.b64encode(frame_bytes).decode() if frame_bytes else None

    if cfg.get("openrouter_key"):
        result = _call_openrouter(cfg["openrouter_key"], prompt, b64)
        if result:
            return result

    if cfg.get("google_key"):
        result = _call_google(cfg["google_key"], prompt, b64)
        if result:
            return result
...
def phase1_learn_game(cfg: dict, frame_bytes: bytes, kb: dict):
    """Run ONCE on first launch to let AI explain the game."""
    ...
    prompt = (
        ...
        "Be specific about pixel positions only if clearly visible. "
        "Focus on what a bot needs to know to play and handle every screen."
    )
...
def phase3_consolidate(cfg: dict, kb: dict):
    """After session ends, distill new observations into permanent rules."""
...
def build_game_prompt(game_state: str, recent_moves: list, kb: dict) -> str:
    ...
    return (
        ...
        f"Look at this screenshot. Output the SINGLE best action — one line only:\n"
        f"  MOVE LEFT\n  MOVE RIGHT\n  JUMP\n  NONE\n  TAP [x] [y]\n\n"
        f"Output NONE only if the path is clearly unobstructed."
    )
...
def main_menu(cfg: dict, kb: dict) -> dict:
    while True:
        ...
        if not cfg.get("openrouter_key") and not cfg.get("google_key"):
            print("\n⚠️  No API keys set — use options 1 or 2 first.")
...
class BunnyBotAI:
    ...
    def start_loop(self):
        print("\n🧠 BunnyBot starting up... Switch to the game!")
        for i in range(5, 0, -1):
            print(f"   Starting in {i}s...", end="\r", flush=True)
            time.sleep(1)
        print("\n🚀 Live! (Ctrl+C to stop)\n")

        # Phase 1 — learn game on very first run
        if self.ai_available and not self.kb.get("game_summary"):
            try:
                phase1_learn_game(self.config, self.get_frame(), self.kb)
            except Exception as e:
                print(f"⚠️  Phase 1 skipped: {e}")
            ...
            while True:
                try:
                    ...
                    if self._fail_streak >= 3:
                        print("❌ ADB connection lost. Check adb connect.")
                        break
